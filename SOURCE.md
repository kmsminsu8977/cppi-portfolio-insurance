...
이 문서는 `cppi-portfolio-insurance` 컨텐츠를 만들 때 참고해야 할 로컬 source inventory입니다. 새 코드, 데이터 예시, 문서, 산출물을 만들 때는 이 파일과 `AGENTS.md`를 먼저 확인합니다.

## 프로젝트 식별

- 프로젝트명: CPPI 포트폴리오 보험 (CPPI Portfolio Insurance)
- 연구 카테고리: Dynamic Portfolio Insurance
- 핵심 질문: 초기 보호수준을 유지하면서 상승장 참여도를 높이려면 multiplier가 어떤 역할을 하는가?

## KAIST-DFMBA 참고 경로

아래 경로는 컨텐츠를 설계할 때 확인할 로컬 참고 위치입니다. 세부 구현은 이 저장소의 목적에 맞게 별도 구조로 정리합니다.

- KAIST-DFMBA/Pension-Fund/Retirement-Investing/
- KAIST-DFMBA/Pension-Fund/Example-Reproducible-Code/
- KAIST-DFMBA/Financial-Data-Analysis/

## 참고 초점

- floor, cushion, multiplier 기반 동적 배분
- 위험자산/안전자산 수익률 경로 적용
- floor breach와 상승 참여도 해석
